...
try:
    import ssl
    HAVE_SSL = True
    logger.debug("SSL模块导入成功")
except Exception as e:
    logger.debug(f"SSL模块导入失败: {e}")
    HAVE_SSL = False
...
class FeishuAdapter:
    def __init__(self, config=None):
        ...
        logger.debug("适配器初始化完成")

    def test_connection(self):
        logger.debug("测试网络连接...")
        try:
            sock = socket.create_connection(('open.feishu.cn', 443 if HAVE_SSL else 80), timeout=5)
            sock.close()
            logger.debug("TCP连接成功")
            return True
        except Exception as e:
            logger.debug(f"TCP连接失败: {e}")
            return False
